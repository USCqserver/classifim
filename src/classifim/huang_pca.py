...
from classifim.bits import popcount64
...
def repack_rectangular_spins_to_uint64(samples, width):
    n_samples, height = samples.shape
    num_words = (width * height + 63) // 64
    zs = np.zeros((n_samples, num_words), dtype=np.uint64)
    word_i = 0
    offset = 0
    for j in range(height):
        cur_row = samples[:, j].astype(np.uint64)
        zs[:, word_i] |= cur_row << offset
        offset += width
        if offset >= 64:
            word_i += 1
            offset -= 64
            if offset > 0:
                zs[:, word_i] |= cur_row >> (width - offset)
    return zs
...
class ComputeKShadowMatrixPipeline:
    ...
    def compute_pca(self, top_components=10):
        ...
        self.pca = pca.reshape(*self.grid_shape, top_components)
